...
export default async function HistoryPage() {
  ...
  const totalCovered = rows.reduce(
    (n, r) => n + coveredCents(r.billTotalCents, r.capCents),
    0,
  );
  const totalExcess = rows.reduce(
    (n, r) => n + excessCents(r.billTotalCents, r.capCents),
    0,
  );
  ...
  return (
    <>
      ...
      <CenteredPage>
          ...
          {rows.length === 0 ? (
            <div className="p-6">
              <EmptyState title="No claims yet">
                Scan the poster at the counter on a Thursday and your claims
                will show up here.
              </EmptyState>
            </div>
          ) : (
            <>
                    ...
                    >
                      <div className="min-w-0">
                        <p className="font-medium text-fg">
                          {formatYmdLong(r.claimDate)}
                        </p>
                        <p className="mt-0.5 text-sm text-fg-muted">
                          <span className="tnum">{localHm(r.claimedAt)}</span> ·
                          bill{" "}
                          <span className="tnum">
                            {formatCents(r.billTotalCents)}
                          </span>
                          {r.carWashCents > 0 ? (
                            <>
                              {" "}
                      ...
                      <div className="shrink-0 text-right">
                        <p className="tnum font-semibold text-fg">
                          {formatCents(covered)}
                        </p>
                        <p className="mt-0.5 text-sm text-fg-muted">
                          {excess > 0 ? (
              ...
              <dl className="flex justify-between gap-4 border-t border-border bg-surface-muted px-6 py-4 text-sm">
                <div>
                  ...
                  <dd className="tnum mt-0.5 text-base font-semibold text-fg">
                    {formatCents(totalCovered)}
                  </dd>
                </div>
                <div className="text-right">
                  <dt className="text-fg-muted">You paid</dt>
                  <dd className="tnum mt-0.5 text-base font-semibold text-fg">
                    {formatCents(totalExcess)}
                  </dd>
                </div>
              </dl>
            </>
        ...
        <EmployeeFooterLink href="/scan" label="Back to scanner" />
      </CenteredPage>
    </>
  );
}
